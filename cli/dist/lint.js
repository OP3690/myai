...
function computeMetricScores(issues) {
    const result = {
        clarity: 100,
        context: 100,
        structure: 100,
...
export function severityColor(s) {
    if (s === "error")
        return "bg-severity-error/15 text-severity-error border-severity-error/30";
    if (s === "warning")
        return "bg-severity-warn/15 text-severity-warn border-severity-warn/30";
...
export function metricTextColor(value) {
    if (value >= 80)
