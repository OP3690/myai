...
const INLINE_CONSTRAINT_PATTERN = /\bit\s+should\s+be\s+(\w+(?:\s+and\s+\w+)*)/gi;
const QUALITY_FILLER_PATTERN = /\b(?:make\s+it\s+|should\s+be\s+|that\s+is\s+)(good|nice|cool|great|awesome|interesting|engaging)\b/gi;
// ─── Task-type detection ──────────────────────────────────────────────────
...
export function detectTaskType(text) {
    ...
    if (/\bwrite\b|\bdraft\b|\bcompose\b/i.test(t))
        return "generative-writing";
    return "general";
}
// ─── Topic extraction ─────────────────────────────────────────────────────
const TOPIC_STRIP_PREFIXES = /^(?:about\s+|on\s+|for\s+|me\s+(?:about|how\s+to)\s+|how\s+to\s+|why\s+|what\s+(?:is|are|do|does)\s+|the\s+)/i;
export function extractTopic(text, taskType) {
    let t = text.trim();
    // Strip imperative verbs at the start
    t = t.replace(/^(?:write|draft|compose|create|build|implement|explain|teach|tutor|describe|analyse|analyze|compare|evaluate|extract|find|identify|translate|convert|summarize|summarise|refactor|recommend|advise|brainstorm|ideate|generate|debug|fix|review|act|pretend|roleplay|simulate|walk\s+me\s+through|learn)\s+/i, "");
...
function inferRole(text, taskType) {
...
function inferFormat(text, taskType) {
    const t = text.toLowerCase();
    if (/\bjson\b/.test(t))
...
function inferLength(text, taskType) {
    const t = text.toLowerCase();
    if (/\b(short|brief|concise|tldr|quick|one[- ]paragraph|one[- ]sentence)\b/.test(t))
        return "under 120 words";
    if (/\b(detailed|comprehensive|thorough|in[- ]depth|exhaustive|long)\b/.test(t))
        return "400–600 words";
    if (taskType === "generative-code")
        return "as long as the code requires — no padding in the explanation";
    if (taskType === "extraction")
        return "no length limit — return all matches";
    if (taskType === "creative")
        return "8–10 distinct options";
    return "under 250 words";
}
function inferAudienceLine(text, taskType) {
    const t = text.toLowerCase();
    ...
    if (taskType === "generative-code")
        return "engineering peers — assume comfort with the language and ecosystem";
    if (taskType === "educational")
        return "intermediate — skip the obvious, but explain anything truly advanced";
    return "intermediate — skip the obvious, but flag anything truly advanced";
}
// ─── Example generation ───────────────────────────────────────────────────
function generateExample(text, taskType, topic) {
    const t = text.toLowerCase();
    switch (taskType) {
...
function cleanTaskText(text) {
    ...
    for (const m of matches) {
        ...
        for (const adj of adjectives) {
            ...
            if (/^(good|nice|cool|great|awesome|interesting|engaging)$/i.test(adj))
                continue;
...
export function improvePrompt(input) {
    ...
    let resolvedConstraint = null;
    for (const c of CONTRADICTION_PAIRS) {
        if (c.a.test(task) && c.b.test(task)) {
            const droppedSrc = (c.keep === "a" ? c.b : c.a).source;
            const cleanupPattern = new RegExp(`(?:\\s+(?:and|but|or|also|yet|but\\s+also)\\s+)?${droppedSrc}|${droppedSrc}(?:\\s+(?:and|but|or|also|yet|but\\s+also)\\s+)?`, "i");
    ...
    const lengthFromConstraint = cleaned.pulledConstraints.find((c) => /\b(short|brief|concise|detailed|comprehensive|thorough|long)\b/i.test(c));
    ...
    const safeguard = wordsInOriginal > 8
...
function extractRoleFromOriginal(text) {
...
export function renderForModel(p, model) {
    const { sections } = p;
    if (model === "claude")
        return renderClaude(sections);
    if (model === "gpt")
        return renderGPT(sections);
    if (model === "gemini")
        return renderGemini(sections);
...
function renderClaude(s) {
    ...
    const constraints = [];
    if (s.length)
        constraints.push(`- Length: ${s.length}`);
    if (s.audience)
        constraints.push(`- Audience: ${s.audience}`);
    if (constraints.length)
        parts.push(`<constraints>\n${constraints.join("\n")}\n</constraints>`);
    if (s.example)
        parts.push(`<example>\n${s.example}\n</example>`);
    if (s.safeguard)
        parts.push(`<safeguard>\n${s.safeguard}\n</safeguard>`);
    return parts.join("\n\n");
}
function renderGPT(s) {
    const parts = [];
    if (s.role)
        parts.push(`**Role**: You are ${s.role}.`);
    parts.push(`## Task\n${s.task}`);
...
function renderPlain(s) {
    const parts = [];
    if (s.role)
        parts.push(`Act as ${s.role}.`);
    parts.push(`Task: ${s.task}`);
    const constraints = [];
    if (s.format)
        constraints.push(`Format: ${s.format}.`);
    if (s.length)
        constraints.push(`Length: ${s.length}.`);
    if (s.audience)
        constraints.push(`Audience: ${s.audience}.`);
    ...
    if (s.example)
        parts.push(s.example);
    if (s.safeguard)
        parts.push(s.safeguard);
    return parts.join("\n\n");
}
export function improvePromptLocal(input) {
