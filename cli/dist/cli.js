...
import { improvePrompt, renderForModel } from "./improve.js";
...
(async () => {
    ...
    const model = modelArgIdx >= 0 && args[modelArgIdx + 1]
        ? args[modelArgIdx + 1]
        : "plain";
    ...
    if (improved.topic)
        console.log(`  ${bold("Topic:")}      ${dim(improved.topic)}`);
    ...
    if (lint.issues.length) {
        ...
        if (lint.issues.length > 10)
            console.log(dim(`    … and ${lint.issues.length - 10} more`));
        console.log("");
    }
